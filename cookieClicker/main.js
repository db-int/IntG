		...
		*/
		...
		if (!EN)
		{
			var adaptWidth = function (node) {
				if (!node || !node.firstChild) return;  // ⬅️ GUARDIA EXTRA

				var el = node.firstChild;
				if (!el || !el.classList) return;       // ⬅️ OTRA GUARDIA
				...
				width = width / 95;
				if (width > 1) {
					var size = parseInt(window.getComputedStyle(el).fontSize) || 12;
					el.style.fontSize = (size * 1 / width) + 'px';
