...
const PLAYERPREFS_KEY_BRAINROTUP = "/idbfs/58db4c007b83bc08104c820b60d9a752/PlayerPrefs";
const RELOAD_FLAG_KEY = "InteractiveBrainrotUpPendingReloadV2";

// -----------------------------
// Parchear PlayerPrefs
// -----------------------------
async function patchBrainrotUpPlayerPrefs() {
  ...
  const INIT_MARKER_KEY = "InteractiveBrainrotUpInitV1";
  ...
  function patchOneRecord(record) {
    ...
    for (const [key, defVal] of Object.entries(DURATION_DEFAULTS)) {
      if (!existing.has(key)) {
        ...
        console.log("[BrainrotUp][IDBFS] Programado añadir duración", key, "=", defVal);
      }
    }
    ...
    if (shouldInitAudio) {
      for (const key of AUDIO_KEYS) {
        ...
        if (info && info.type === "float" && Math.abs(info.value - 1) < 1e-6) {
          ...
          changed = true;
          console.log("[BrainrotUp][IDBFS] Audio default", key, ": 1.0 -> 0.5");
        }
      }
    }
    ...
    if (!initMarker && (shouldInitAudio || entriesToAppend.length > 0)) {
      ...
      console.log("[BrainrotUp][IDBFS] Añadiendo marker", INIT_MARKER_KEY);
    }
    ...
    if (entriesToAppend.length > 0) {
      ...
      for (const entry of entriesToAppend) {
        ...
        console.log("[BrainrotUp][IDBFS] Escrito nuevo PlayerPref", key, "=", defVal);
      }
  ...
  if (!window.indexedDB) {
    console.warn("[BrainrotUp][IDBFS] indexedDB no disponible.");
    return "no_idb";
  }
  ...
  return new Promise((resolve) => {
    ...
    openReq.onerror = () => {
      console.warn("[BrainrotUp][IDBFS] No se pudo abrir /idbfs:", openReq.error);
      resolve("no_idb");
    };
    ...
    openReq.onsuccess = (event) => {
      ...
      if (!db.objectStoreNames.contains("FILE_DATA")) {
        console.warn("[BrainrotUp][IDBFS] DB sin FILE_DATA todavía.");
        db.close();
        resolve("no_idb");
        return;
      }
      ...
      cursorReq.onerror = () => {
        console.warn("[BrainrotUp][IDBFS] Error recorriendo FILE_DATA:", cursorReq.error);
      ...
      cursorReq.onsuccess = (e) => {
        ...
        if (!cursor) {
          console.log("[BrainrotUp][IDBFS] Fin de FILE_DATA. ¿Cambios?", patched);
          resolve(patched ? "patched" : (sawPlayerPrefs ? "no_changes" : "no_playerprefs"));
          return;
        }
        ...
        // Sólo el PlayerPrefs específico de Brainrot Up
        if (key === PLAYERPREFS_KEY_BRAINROTUP) {
          sawPlayerPrefs = true;
          console.log("[BrainrotUp][IDBFS] Revisando PlayerPrefs:", key);
...
function waitForBrainrotUpPlayerPrefsInIdbfs(maxMs = 20000, intervalMs = 1000) {
  ...
  return new Promise((resolve) => {
    const tick = () => {
      ...
      if (Date.now() - start > maxMs) {
        console.warn("[BrainrotUp][IDBFS] Timeout esperando PlayerPrefs de Brainrot Up.");
        resolve(false);
        return;
      }
      ...
      openReq.onsuccess = (event) => {
        ...
        cursorReq.onsuccess = (e) => {
          ...
          if (!cursor) {
            db.close();
            if (found) {
              console.log("[BrainrotUp][IDBFS] Detectado PlayerPrefs de Brainrot Up.");
              resolve(true);
            } else {
              setTimeout(tick, intervalMs);
            }
...
(async () => {
  ...
  try {
    patchResult = await patchBrainrotUpPlayerPrefs();
    console.log("[BrainrotUp] Resultado del patch:", patchResult);
  } catch (e) {
    console.warn("[BrainrotUp] Error parchando PlayerPrefs (se ignora):", e);
    patchResult = "error";
  }

  try {
    unityInstance = await createUnityInstance(document.querySelector("#unityCanvas"), {
      dataUrl: "brainrot-up.data",
      frameworkUrl: "brainrot-up.framework.js",
      codeUrl: "brainrot-up.wasm",
      streamingAssetsUrl: "StreamingAssets",
      companyName: "Interactive",
      productName: "Brainrot Up",
      productVersion: "1.0",
      // autoSyncPersistentDataPath: true, // opcional si tu loader lo soporta
    });

    console.log("[BrainrotUp] Unity cargado correctamente.");
    ...
    if (patchResult === "no_idb" || patchResult === "no_playerprefs") {
      ...
      if (!yaRecargado) {
        console.log("[BrainrotUp] Aún no hay PlayerPrefs. Esperando a que el juego cree PlayerPrefs para recargar una vez...");

        waitForBrainrotUpPlayerPrefsInIdbfs().then((ok) => {
          if (!ok) {
            console.warn("[BrainrotUp] No se detectó PlayerPrefs de Brainrot Up a tiempo. No se recarga para evitar bucles raros.");
            return;
          }

          console.log("[BrainrotUp] PlayerPrefs de Brainrot Up detectado. Marcando recarga pendiente y recargando la página.");
          localStorage.setItem(RELOAD_FLAG_KEY, "1");
          location.reload();
        });
      } else {
        console.warn("[BrainrotUp] Ya se recargó una vez y aún así no se pudo parchear. No se vuelve a recargar.");
      }
    } else {
    ...
    console.error("[BrainrotUp] Error cargando Unity:", err);
  }
})();

function sendEvent(jsonObj) {
  if (!unityInstance) {
    console.warn("[BrainrotUp] Unity no está listo.");
    return;
  }
...
function triggerInteraction(interactionKey, times = 1, forcedUser) {
  console.log("[BrainrotUp] TriggerInteraction:", interactionKey, "x", times);
...
// Interacciones
// -----------------------------
if (window.brainrotUp && typeof window.brainrotUp.onMessage === "function") {
  window.brainrotUp.onMessage((msg) => {
    ...
    switch (msg.type) {
      ...
      default:
        console.log("[BrainrotUp] Mensaje desconocido desde main:", msg);
    }
  });
}
