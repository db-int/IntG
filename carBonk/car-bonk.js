...
const PLAYERPREFS_KEY_CARBONK = "/idbfs/28b85574d9aea43ad2106f566a4bde70/PlayerPrefs";
const RELOAD_FLAG_KEY = "InteractiveCarBonkPendingReloadV2";

// -----------------------------
// Parchear PlayerPrefs
// -----------------------------
async function patchCarBonkPlayerPrefs() {
  // Duraciones que quieres crear si no existen (NO tocar si ya existen)
  const DURATION_DEFAULTS = {
    DrunkVisionDuration: 1,
    InvertedControlsDuration: 1,
    SlipperyGroundDuration: 1,
  };

  // Volúmenes por defecto: solo bajar a 0.5 en el primer arranque
  const AUDIO_KEYS = [
    "GameAudioVolume",
    "GameMusicVolume",
    "GameSoundVolume",
  ...
  const INIT_MARKER_KEY = "InteractiveCarBonkInitV1"; // nuestra marca
  ...
  function patchOneRecord(record) {
    ...
    const existing = new Map(); // name -> { type: 'float', offset, value }

    // "UnityPrf" header + metadatos ≈ 16 bytes
    let pos = 16;
    ...
    for (const [key, defVal] of Object.entries(DURATION_DEFAULTS)) {
      if (!existing.has(key)) {
        ...
        console.log("[CarBonk][IDBFS] Programado añadir duración", key, "=", defVal);
      }
    }
    ...
    if (shouldInitAudio) {
      for (const key of AUDIO_KEYS) {
        ...
        if (info && info.type === "float" && Math.abs(info.value - 1) < 1e-6) {
          ...
          changed = true;
          console.log("[CarBonk][IDBFS] Audio default", key, ": 1.0 -> 0.5");
        }
      }
    }
    ...
    if (!initMarker && (shouldInitAudio || entriesToAppend.length > 0)) {
      ...
      console.log("[CarBonk][IDBFS] Añadiendo marker", INIT_MARKER_KEY);
    }
    ...
    if (entriesToAppend.length > 0) {
      ...
      for (const entry of entriesToAppend) {
        ...
        console.log("[CarBonk][IDBFS] Escrito nuevo PlayerPref", key, "=", defVal);
      }
  ...
  if (!window.indexedDB) {
    console.warn("[CarBonk][IDBFS] indexedDB no disponible.");
    return "no_idb";
  }
  ...
  return new Promise((resolve) => {
    ...
    openReq.onerror = () => {
      console.warn("[CarBonk][IDBFS] No se pudo abrir /idbfs:", openReq.error);
      resolve("no_idb");
    };
    ...
    openReq.onsuccess = (event) => {
      ...
      if (!db.objectStoreNames.contains("FILE_DATA")) {
        console.warn("[CarBonk][IDBFS] DB sin FILE_DATA todavía.");
        db.close();
        resolve("no_idb");
        return;
      }
      ...
      cursorReq.onerror = () => {
        console.warn("[CarBonk][IDBFS] Error recorriendo FILE_DATA:", cursorReq.error);
      ...
      cursorReq.onsuccess = (e) => {
        ...
        if (!cursor) {
          console.log("[CarBonk][IDBFS] Fin de FILE_DATA. ¿Cambios?", patched);
          resolve(patched ? "patched" : (sawPlayerPrefs ? "no_changes" : "no_playerprefs"));
          return;
        }
        ...
        // Sólo nos interesa el PlayerPrefs ESPECÍFICO de CarBonk
        if (key === PLAYERPREFS_KEY_CARBONK) {
          sawPlayerPrefs = true;
          console.log("[CarBonk][IDBFS] Revisando PlayerPrefs:", key);
...
function waitForCarBonkPlayerPrefsInIdbfs(maxMs = 20000, intervalMs = 1000) {
  ...
  return new Promise((resolve) => {
    const tick = () => {
      ...
      if (Date.now() - start > maxMs) {
        console.warn("[CarBonk][IDBFS] Timeout esperando PlayerPrefs de CarBonk.");
        resolve(false);
        return;
      }
      ...
      openReq.onsuccess = (event) => {
        ...
        cursorReq.onsuccess = (e) => {
          ...
          if (!cursor) {
            db.close();
            if (found) {
              console.log("[CarBonk][IDBFS] Detectado PlayerPrefs de CarBonk.");
              resolve(true);
            } else {
              setTimeout(tick, intervalMs);
            }
...
(async () => {
  ...
  try {
    patchResult = await patchCarBonkPlayerPrefs();
    console.log("[CarBonk] Resultado del patch:", patchResult);
  } catch (e) {
    console.warn("[CarBonk] Error parchando PlayerPrefs (se ignora):", e);
    patchResult = "error";
  }

  try {
    unityInstance = await createUnityInstance(document.querySelector("#unityCanvas"), {
      dataUrl: "car-bonk.data",
      frameworkUrl: "car-bonk.framework.js",
      codeUrl: "car-bonk.wasm",
      streamingAssetsUrl: "StreamingAssets",
      companyName: "Interactive",
      productName: "Car Bonk",
      productVersion: "1.0",
      // Opcional: ayuda a sincronizar automáticamente
      // autoSyncPersistentDataPath: true,  // si tu wrapper lo soporta
    });

    console.log("[CarBonk] Unity cargado correctamente.");
    ...
    if (patchResult === "no_idb" || patchResult === "no_playerprefs") {
      ...
      if (!yaRecargado) {
        console.log("[CarBonk] Aún no hay PlayerPrefs. Esperando a que el juego cree PlayerPrefs para recargar una vez...");

        waitForCarBonkPlayerPrefsInIdbfs().then((ok) => {
          if (!ok) {
            console.warn("[CarBonk] No se detectó PlayerPrefs de CarBonk a tiempo. No se recarga para evitar bucles raros.");
            return;
          }

          console.log("[CarBonk] PlayerPrefs de CarBonk detectado. Marcando recarga pendiente y recargando la página.");
          localStorage.setItem(RELOAD_FLAG_KEY, "1");
          location.reload();
        });
      } else {
        console.warn("[CarBonk] Ya se recargó una vez y aún así no se pudo parchear. No se vuelve a recargar.");
      }
    } else {
    ...
    console.error("[CarBonk] Error cargando Unity:", err);
  }
})();
...
function triggerInteraction(interactionKey, times = 1, forcedUser) {
  console.log("[CarBonk] TriggerInteraction:", interactionKey, "x", times);
...
// Interacciones
// -----------------------------
if (window.carBonk && typeof window.carBonk.onMessage === "function") {
  window.carBonk.onMessage((msg) => {
    ...
    switch (msg.type) {
      ...
      default:
        console.log("[CarBonk] Mensaje desconocido desde main:", msg);
    }
  });
}
